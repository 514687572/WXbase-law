...
<!--     <div id="c-head">
       	...
       	<c:if test="${requestScope.dateList!=null && requestScope.dateList!=''}" var="condition" scope="request">
			<c:forEach var="adList" items="${requestScope.dateList}" varStatus="list">
				<c:if test="${adList.attentionDate==1}" var="condition" scope="request">
			 		<div class="c-text" onclick="getDetail('${adList.dateId}')" style="background-color: #FED700;">
			            <div class="c-wrap">
		            	...
		            	<c:if test="${adList.gc==false}" var="condition" scope="request">
		                	<p class="c-yytime">${adList.remark3}&nbsp;农历:${adList.lunarCh}</p>
		                </c:if>
			            <c:if test="${adList.gc==true}" var="condition" scope="request">
		                	<p class="c-yytime">${adList.remark3} </p>
		                </c:if>
			        </div>
				</c:if>
				<c:if test="${adList.attentionDate==0}" var="condition" scope="request">
			 		<div class="c-text" onclick="getDetail('${adList.dateId}')" style="background-color: #24B8EB;">
			            <div class="c-wrap">
			                <span class="c-name">${adList.dateTitle}</span>
			                ...
			                <c:if test="${adList.gc==false}" var="condition" scope="request">
			                	<p class="c-yytime">农历:${adList.lunarCh}</p>
			                </c:if>
			            </div>
			        </div>
				</c:if>
    ...
    <div id="cadd" onclick="addDate();"></div>
</div>
<script>
document.title="我的纪念日";
 	...
 	function addDate(){
 		window.location.href='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxd648cfd0204906c1&redirect_uri=http%3A%2F%2Fweixinmap.com%2Fwechat%2Fportal%2Fauth&response_type=code&scope=snsapi_userinfo&state=addDate#wechat_redirect'; 
 	}
